...
import thermal_runaway_ICI_plot_gui_1
...
class Dataset():
    ...
    '''
    def __init__(self, directory='', label='', coords=[]):
...
def create_datasets(values):
    '''
    inputs:
        values (dict): contains user-inputted information for Datasets from GUI
    
    returns:
        datasets (list): contains all created Datasets
    '''
    datasets = []
    for key in values:
        if 'label_' in key:
            if values[key] == '':
                ...
                n = key.replace('label_', '')
                datasets.append(Dataset(values['dir_'+n], values['label_'+n],
                    [values['tl_'+n],values['br_'+n]]))
    return datasets
...
def data_plot(datasets):
    '''
    inputs:
        datasets (list): Datasets created from user inputs
...
datasets = create_datasets(thermal_runaway_ICI_plot_gui_1.values)
y_min, y_max = data_plot(datasets)
